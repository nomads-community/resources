...
from resources.lib.general import produce_dir
...
def config_root_logger(log_dir: Path, verbose=False) -> None:
    ...
    # Reconfigure the stream to handle encoding issues
    try:
        console_handler.stream.reconfigure(
            encoding="utf-8", errors="replace"
        )  # Or errors='ignore'
    except AttributeError:  # If stream does not have reconfigure method
        pass
